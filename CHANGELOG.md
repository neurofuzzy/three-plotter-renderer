...
### Added
- **Async frame-budgeted rendering** - `renderGPULayers()` now returns a Promise and yields to browser between frames
- **Progress callback** - `hatchOptions.progressCallback` for render progress updates (0-1)
- **Zoom-invariant hatching** - Hatch density stays consistent relative to model regardless of zoom level
- **Inset control in example UIs** - Added UI control for `insetPixels` in both examples

### Changed
- Default `insetPixels` reduced from 3 to 2
- Hatch boundary erosion now uses mathematical polygon insetting instead of pixel-based erosion
- `renderGPULayers()` is now async - callers should `await` it

### Fixed
- Fixed hole hatching - holes are now properly clipped against their parent regions
- Fixed hatch erosion not working (was using wrong boundary for clipping)
- Fixed hatching only appearing on X-axis-facing faces when zoom-invariant scaling was enabled
